...
public class VStoreFile implements Serializable {
     ...
     */
    private List<String> mStoredNodeIDs = new ArrayList<>();
    ...
    public String getMainNodeId() {
        if (mStoredNodeIDs.size() > 0) {
            return mStoredNodeIDs.get(0);
        }
        return null;
    }
     ...
     */
    public void setMainNodeId(String uuid) { mStoredNodeIDs.add(0, uuid); }
    ...
    /**
     * Adds more identifiers of storage nodes, on which the file is stored
     * @param node_ids A list of storage node identifiers
     */
    public void addMoreStoredNodeIds(List<String> node_ids) {
        if(node_ids == null || node_ids.size() == 0) { return; }
        mStoredNodeIDs.addAll(node_ids);
    }

    /**
     * Adds another identifier of a storage node, on which the file is stored
     * @param node_id A node identifier.
     */
    public void addStoredNodeId(String node_id) {
        if(!mStoredNodeIDs.contains(node_id)) {
            mStoredNodeIDs.add(node_id);
        }
    }
    /**
     * Sets the list of identifiers of storage nodes, on which the file is stored,
     * to the given list.
     * @param node_ids The list of storage node identifiers.
     */
    public void setStoredNodeIds(List<String> node_ids) {
        if(node_ids == null || node_ids.size() == 0) { return; }
        mStoredNodeIDs.clear();
        mStoredNodeIDs.addAll(node_ids);
    }

    /**
     * @return A list of all identifiers of storage nodes, on which the file is stored.
     */
    public final List<String> getStoredNodeIds() {
        return mStoredNodeIDs;
    }

    /**
     * @return A list of all identifiers of storage nodes, on which the file is stored. In json format.
     */
    public JSONArray getStoredNodeIdsJson() {
        JSONArray json = new JSONArray();
        for(String node_id : mStoredNodeIDs)
        {
            json.add(node_id);
        }
        return json;
    ...
    /**
     * Sets the list of identifiers of storage nodes, on which the file is stored,
     * to the identifiers given in the json array
     * @param storedNodeJson The json array of storage node identifiers.
     */
    public void setStoredNodeIDs(JSONArray storedNodeJson) {
        if(storedNodeJson == null) { return; }
        mStoredNodeIDs = new ArrayList<>();
        for(int i = 0; i<storedNodeJson.size(); ++i) {
            mStoredNodeIDs.add((String)storedNodeJson.get(i));
        }
    }
}
