...
import static vstore.framework.communication.ApiConstants.StorageNode.ROUTE_FILE;
import static vstore.framework.communication.ApiConstants.StorageNode.ROUTE_FILE_DELETE;
import static vstore.framework.communication.ApiConstants.StorageNode.ROUTE_FILE_METADATA_FULL;
import static vstore.framework.communication.ApiConstants.StorageNode.ROUTE_FILE_METADATA_LIGHT;
...
public class NodeInfo implements Serializable {
     ...
     * 
     * @param node The json object which describes the node info.
     */
    public NodeInfo(JSONObject node) {
        if(node != null) 
        {
            		...
            		&& node.containsKey("type") && node.containsKey("location")) 
            {
                ...
                mPort = JsonUtils.getIntFromJson("port", node, 0);
                try 
                {
                    mNodeType = NodeType.valueOf(JsonUtils.getStringFromJson("type", node, NodeType.UNKNOWN.name()));
                } 
                ...
                setBandwidthDown(JsonUtils.getIntFromJson("bandwidthDown", node, 0));
                setBandwidthUp(JsonUtils.getIntFromJson("bandwidthUp", node, 0));
                return;
            }
        }
     ...
     */
    public String getIdentifier() { return mUUID; }
     ...
     * @param phoneID The phone ID of the phone (see {@link IdentifierUtils#getDeviceIdentifier()} ()}).
     ...
     */
    public float getGeographicDistanceTo(VLatLng latlng) {
    ...
    public void setDistanceMetric(float f) {
        distanceMetric = f;
    }
}
